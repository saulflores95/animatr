...
function heightSlider(value) {
   try {
      ...
      if (currentFig.id.includes("triangle") || currentFig.id.includes("octagon")) {
...
function borderStyleChange() {
   try {
      if (currentFig.id.includes("triangle") || currentFig.id.includes("octagon")) {} else {
...
function loadProps(div) {
   ...
   borderWidthField.setAttribute('readonly', true);
